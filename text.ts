...
export function formatNumber(numberer: string | number, commaDigits: number, decimalSep: string, thousandSep: string) {
    var commaDigits = isNaN(commaDigits = Math.abs(commaDigits)) ? 2 : commaDigits,
        ...
        strOutput = numberer < 0 ? "-" : "",
        intNr = parseInt(numberer = Math.abs(+numberer || 0).toFixed(commaDigits)) + "",
