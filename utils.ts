...
let logger: winstonGlobal.Logger = null; // ensure we have defined this before functions using it
...
export function toBase64(text: string, from:BufferEncoding  = 'utf8') {
    if (from === "base64")
        return base64url.escape(text)
    return base64url.escape(Buffer.from(text, from).toString('base64'))
}

export function fromBase64(text: string, to:BufferEncoding = 'utf8') {
    return Buffer.from(base64url.unescape(text), 'base64').toString(to)
}

export function toBase32(text: string, from:BufferEncoding = 'utf8') {
    return base32.encode(Buffer.from(text, from))
}
