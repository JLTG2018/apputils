...
if (process.env.IS_CHILD) {
    ...
    winstonLogger = winston.createLogger({ transports: [
    ...
    if (logfile !== undefined && logfile != '' && !process.env.IS_CHILD) {
        ...
        new winston.transports.File({
            level: nconf.get('debug') === true ? 'debug' : 'info',
            filename: logfile,
            format: winston.format.combine(nconf.get('jsonLog') === true ? winston.format.json() : null, winston.format.timestamp(), winston.format.prettyPrint()),
...
const myWinstonLogger = winston.loggers;
// forward console.log() calls to winston.log('info', args)
winstonLogger.log = function () {
    const forwardTo = 'info';
    if (arguments.length === 0)
        return myWinstonLogger.prototype.log.apply(this, arguments); // shouldn't be called at all like this
    else if (arguments.length === 1) // winston doesn't have log('foo'), redirect it to log('info', 'foo')
        return myWinstonLogger.prototype.log.call(this, forwardTo, arguments[0]);
    let firstArg = arguments[0];
    let levels = ['silly', 'debug', 'verbose', 'info', 'warn', 'error'];
    for (let level of levels) {
        if (firstArg === level)
            return myWinstonLogger.prototype.log.apply(this, arguments); // forward the original call
    }
    let newArgs = [forwardTo]; // forward it to log('info', arguments)
    for (let i = 0; i < arguments.length; i++)
        newArgs.push(arguments[i]);
    return myWinstonLogger.prototype.log.apply(this, newArgs);
};
