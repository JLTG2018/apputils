...
function formatNumber(numberer, commaDigits, decimalSep, thousandSep) {
    var commaDigits = isNaN(commaDigits = Math.abs(commaDigits)) ? 2 : commaDigits, decimalSep = decimalSep == undefined ? "." : decimalSep, thousandSep = thousandSep == undefined ? "," : thousandSep, strOutput = numberer < 0 ? "-" : "", intNr = parseInt(numberer = Math.abs(+numberer || 0).toFixed(commaDigits)) + "", thousands = intNr.length > 3 ? intNr.length % 3 : 0;
    return strOutput + (thousands ? intNr.substr(0, thousands) + thousandSep : "") + intNr.substr(thousands).replace(/(\d{3})(?=\d)/g, "$1" + thousandSep) + (commaDigits ? decimalSep + Math.abs(parseInt(numberer) - parseInt(intNr)).toFixed(commaDigits).slice(2) : "");
}
