...
//const utils = global.__legacyModules ? require('strictutils')() : require('@JLTG2018/apputils')
...
let derefLink = (urlObject: url.Url, link) => {
...
let isIgnoredLink = (urlObj: url.Url) => {
...
export function setFilteredUrlParts(filteredArr) {
    filteredParts = filteredArr
}

export function extractAllLinks(html, cb = null) {
    let links = getExternalLinks(html, "")
    links = utils.getUniqueUrls(links)
    cb && cb(null, links)
}

export function extractHosterLinks(html, decrypt = false, cb = null) {
...
export function extractCrypterLinks(html, decrypt = false, cb = null) {
    if (decrypt === true)
...
export function exctractCustomLinks(html, protocol = 'http', domain = '', cb = null) {
    let htmlLower = html.toLocaleLowerCase()
    if (htmlLower.length !== html.length)
        html = htmlLower // should never happen
